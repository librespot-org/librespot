...
impl PlayerEvent {
	pub fn get_play_request_id(&self) -> Option<u64> {
		...
		match self {
			...
			| Stopped { play_request_id, .. }
			| PositionCorrection { play_request_id, .. }
			| Seeked { play_request_id, .. } => Some(*play_request_id),
			_ => None,
		}
	}
}
	...
	fn parse_from_ogg<T: Read + Seek>(mut file: T) -> io::Result<NormalisationData> {
		...
		if newpos != SPOTIFY_NORMALIZATION_HEADER_START_OFFSET {
			error!(
				...
				SPOTIFY_NORMALIZATION_HEADER_START_OFFSET,
				newpos
			);
...
impl Player {
		...
		where F: FnOnce() -> Box<dyn Sink> + Send + 'static
	{
		...
		if config.normalisation {
			...
			debug!("Normalisation Pregain: {:.1} dB", config.normalisation_pregain_db);
			debug!("Normalisation Threshold: {:.1} dBFS", config.normalisation_threshold_dbfs);
			...
			if config.normalisation_method == NormalisationMethod::Dynamic {
				...
				debug!(
					...
					coefficient_to_duration(config.normalisation_attack_cf).as_secs_f64() * 1000.0
				);
				debug!(
					"Normalisation Release: {:.0} ms",
					coefficient_to_duration(config.normalisation_release_cf).as_secs_f64() * 1000.0
				);
		...
		match new_state {
			Paused {
				...
				*self = Playing {
					...
					bytes_per_second,
					stream_position_ms,
					reported_nominal_start_time: Instant::now().checked_sub(
...
impl PlayerTrackLoader {
	...
	fn stream_data_rate(&self, format: AudioFileFormat) -> Option<usize> {
		let kbps = match format {
			...
			AudioFileFormat::OGG_VORBIS_320 => 40.0,
			AudioFileFormat::MP3_256 => 32.0,
			AudioFileFormat::MP3_320 => 40.0,
			AudioFileFormat::MP3_160 => 20.0,
			AudioFileFormat::MP3_96 => 12.0,
			AudioFileFormat::MP3_160_ENC => 20.0,
			AudioFileFormat::AAC_24 => 3.0,
			AudioFileFormat::AAC_48 => 6.0,
			AudioFileFormat::AAC_160 => 20.0,
			AudioFileFormat::AAC_320 => 40.0,
			AudioFileFormat::MP4_128 => 16.0,
			AudioFileFormat::OTHER5 => 40.0,
			AudioFileFormat::FLAC_FLAC => 112.0, // assume 900 kbit/s on average
			AudioFileFormat::XHE_AAC_12 => 1.5,
			AudioFileFormat::XHE_AAC_16 => 2.0,
			AudioFileFormat::XHE_AAC_24 => 3.0,
			AudioFileFormat::FLAC_FLAC_24BIT => 3.0,
		};
		let data_rate: f32 = kbps * 1024.0;
	...
	async fn load_track(
		...
		let audio_item = match AudioItem::get_file(&self.session, spotify_id).await {
			Ok(audio) =>
				match self.find_available_alternative(audio).await {
					Some(audio) => audio,
					None => {
						warn!("<{}> is not available", spotify_id.to_uri().unwrap_or_default());
						return None;
					}
				}
			Err(e) => {
		...
		info!("Loading <{}> with Spotify URI <{}>", audio_item.name, audio_item.uri);
			...
			})
		{
			Some(t) => t,
			None => {
				warn!("<{}> is not available in any supported format", audio_item.name);
				return None;
			}
		};
		...
		loop {
			...
			let (offset, mut normalisation_data) = if is_ogg_vorbis {
				...
				let normalisation_data = NormalisationData::parse_from_ogg(
					&mut decrypted_file
				).ok();
			...
			let audio_file = match
				Subfile::new(decrypted_file, offset, stream_loader_controller.len() as u64)
			{
			...
			let mut decoder = match decoder_type {
				Ok(decoder) => decoder,
				Err(e) if is_cached => {
					warn!("Unable to read cached audio file: {}. Trying to download it.", e);
			...
			let position_ms = if position_ms > duration_ms {
				warn!(
					"Invalid start position of {} ms exceeds track's duration of {} ms, starting track from the beginning",
					position_ms,
					duration_ms
				);
				0
...
impl Future for PlayerInternal {
	...
	fn poll(mut self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<()> {
		...
		loop {
			...
			if let PlayerPreload::Loading { ref mut loader, track_id } = self.preload {
				match loader.as_mut().poll(cx) {
					Poll::Ready(Ok(loaded_track)) => {
					...
					Poll::Ready(Err(_)) => {
						...
						if
							let
							| PlayerState::Playing { play_request_id, .. }
			...
			if self.state.is_playing() {
					...
					} = self.state
				{
					match decoder.next_packet() {
								...
								if !passthrough {
									match packet.samples() {
										Ok(_) => {
											...
											let notify_about_position = match
												*reported_nominal_start_time
											{
												...
												Some(reported_nominal_start_time) => {
													...
													if packet_position.skipped {
														if
															let Some(ahead) =
																new_stream_position.checked_sub(
													...
													if
														let Some(lag) = now.checked_duration_since(
															reported_nominal_start_time
														)
													{
														if
										...
										Err(e) => {
											error!(
												"Skipping to next track, unable to decode samples for track <{:?}>: {:?}",
												track_id,
												e
											);
						...
						Err(e) => {
							error!(
								"Skipping to next track, unable to get next packet for track <{:?}>: {:?}",
								track_id,
								e
							);
							self.send_event(PlayerEvent::EndOfTrack {
...
impl PlayerInternal {
	...
	fn handle_player_stop(&mut self) {
		match self.state {
			| PlayerState::Playing { track_id, play_request_id, .. }
			| PlayerState::Paused { track_id, play_request_id, .. }
			| PlayerState::EndOfTrack { track_id, play_request_id, .. }
			| PlayerState::Loading { track_id, play_request_id, .. } => {
	...
	fn handle_play(&mut self) {
		match self.state {
			PlayerState::Paused { track_id, play_request_id, stream_position_ms, .. } => {
			...
			PlayerState::Loading { ref mut start_playback, .. } => {
				*start_playback = true;
			}
	...
	fn handle_pause(&mut self) {
		match self.state {
			...
			PlayerState::Playing { track_id, play_request_id, stream_position_ms, .. } => {
	...
	fn handle_packet(
		...
		match packet {
								...
								*sample *= normalisation_factor;
									...
									self.normalisation_peak > 0.0
								{
									...
									self.normalisation_peak =
										attack_cf * self.normalisation_peak -
			...
			None => {
				...
				if let PlayerState::EndOfTrack { track_id, play_request_id, .. } = self.state {
					self.send_event(PlayerEvent::EndOfTrack {
						track_id,
						play_request_id,
					})
	...
	fn start_playback(
		...
		let normalisation_factor = NormalisationData::get_factor(
			&config,
	...
	fn handle_command_load(
		...
		let play_request_id = play_request_id_option.unwrap_or(
			self.play_request_id_generator.get()
		);
		...
		if let PlayerState::EndOfTrack { track_id: previous_track_id, .. } = self.state {
		...
		if let PlayerPreload::Ready { track_id: loaded_track_id, .. } = self.preload {
			if track_id == loaded_track_id {
				let preload = std::mem::replace(&mut self.preload, PlayerPreload::None);
				if let PlayerPreload::Ready { track_id, mut loaded_track } = preload {
					if position_ms != loaded_track.stream_position_ms {
						// This may be blocking
						loaded_track.stream_position_ms = loaded_track.decoder.seek(position_ms)?;
					}
	...
	fn handle_command_preload(&mut self, track_id: SpotifyId) {
		...
		if
			let
			| PlayerPreload::Loading { track_id: currently_loading, .. }
			| PlayerPreload::Ready { track_id: currently_loading, .. } = self.preload
		{
		...
		if
			let
			| PlayerState::Playing { track_id: current_track_id, .. }
			| PlayerState::Paused { track_id: current_track_id, .. }
			| PlayerState::EndOfTrack { track_id: current_track_id, .. } = self.state
		{
			if current_track_id == track_id {
	...
	fn handle_command_seek(&mut self, position_ms: u32) -> PlayerResult {
		...
		if let PlayerState::Loading { track_id, play_request_id, start_playback, .. } = self.state {
		...
		if let PlayerState::Playing { ref mut reported_nominal_start_time, .. } = self.state {
			*reported_nominal_start_time = Instant::now().checked_sub(
				Duration::from_millis(position_ms as u64)
			);
		}
	...
	fn handle_command(&mut self, cmd: PlayerCommand) -> PlayerResult {
		debug!("command={:?}", cmd);
		match cmd {
			PlayerCommand::Load { track_id, play, position_ms } =>
			...
			PlayerCommand::SetSinkEventCallback(callback) => {
				self.sink_event_callback = callback;
			}
			...
			PlayerCommand::EmitSessionConnectedEvent { connection_id, user_name } =>
				self.send_event(PlayerEvent::SessionConnected {
					connection_id,
					user_name,
				}),

			PlayerCommand::EmitSessionDisconnectedEvent { connection_id, user_name } =>
			...
			PlayerCommand::EmitFilterExplicitContentChangedEvent(filter) => {
				...
				if filter {
					if
						let
						| PlayerState::Playing { track_id, play_request_id, is_explicit, .. }
						| PlayerState::Paused { track_id, play_request_id, is_explicit, .. } =
							self.state
					{
						if is_explicit {
							warn!(
	...
	fn send_event(&mut self, event: PlayerEvent) {
		self.event_senders.retain(|sender| sender.send(event.clone()).is_ok());
	}
	...
	fn preload_data_before_playback(&mut self) -> PlayerResult {
		if
			let PlayerState::Playing { bytes_per_second, ref mut stream_loader_controller, .. } =
				self.state
		{
			...
			let wait_for_data_length = (read_ahead_during_playback.as_secs_f32() *
				(bytes_per_second as f32)) as usize;
...
impl fmt::Debug for PlayerCommand {
	fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
		match self {
			PlayerCommand::Load { track_id, play, position_ms, .. } =>
				f.debug_tuple("Load").field(&track_id).field(&play).field(&position_ms).finish(),
			...
			PlayerCommand::EmitVolumeChangedEvent(volume) =>
				f.debug_tuple("EmitVolumeChangedEvent").field(&volume).finish(),
			PlayerCommand::SetAutoNormaliseAsAlbum(setting) =>
				f.debug_tuple("SetAutoNormaliseAsAlbum").field(&setting).finish(),
			PlayerCommand::EmitFilterExplicitContentChangedEvent(filter) =>
				f.debug_tuple("EmitFilterExplicitContentChangedEvent").field(&filter).finish(),
			PlayerCommand::EmitSessionConnectedEvent { connection_id, user_name } =>
				f
			...
			PlayerCommand::EmitSessionClientChangedEvent {
				...
				f
					...
					.finish(),
			PlayerCommand::EmitShuffleChangedEvent(shuffle) =>
				f.debug_tuple("EmitShuffleChangedEvent").field(&shuffle).finish(),
			PlayerCommand::EmitRepeatChangedEvent { context, track } =>
				f.debug_tuple("EmitRepeatChangedEvent").field(&context).field(&track).finish(),
			PlayerCommand::EmitAutoPlayChangedEvent(auto_play) =>
				f.debug_tuple("EmitAutoPlayChangedEvent").field(&auto_play).finish(),
		}
	}
}
...
impl fmt::Debug for PlayerState {
	fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
		...
		match *self {
			...
			Loading { track_id, play_request_id, .. } =>
				f
					.debug_struct("Loading")
					.field("track_id", &track_id)
					.field("play_request_id", &play_request_id)
					.finish(),
			Paused { track_id, play_request_id, .. } =>
				f
					.debug_struct("Paused")
					.field("track_id", &track_id)
					.field("play_request_id", &play_request_id)
					.finish(),
			Playing { track_id, play_request_id, .. } =>
				f
					.debug_struct("Playing")
					.field("track_id", &track_id)
					.field("play_request_id", &play_request_id)
					.finish(),
			EndOfTrack { track_id, play_request_id, .. } =>
				f
		...
		let pos = match pos {
			...
			SeekFrom::End(offset) => {
				if (self.length as i64) - offset < (self.offset as i64) {
					return Err(
						io::Error::new(io::ErrorKind::InvalidInput, "newpos would be < self.offset")
					);
				}
				pos
			}
...
impl<R> MediaSource for Subfile<R> where R: Read + Seek + Send + Sync {
