...
use futures::io::{AsyncReadExt, AsyncWriteExt, WriteAll};
use http::Uri;
use std::error::Error;
use std::io;
use std::str::FromStr;
...
enum ProxyState<'a, T> {
    ...
    ProxyResponse(Read<'a, T>), //TODO what happens to thhe Window ? T, Window<Vec<u8>
}
        ...
        loop {
            self.state = match self.state {
                ProxyConnect(ref mut write) => {
                    ...
                    let read = connection.read(window);
                    ProxyResponse(read)
                }
